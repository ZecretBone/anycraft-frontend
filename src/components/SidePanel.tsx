...
//   apiGetBaseElements,
//   apiGetChallenges,
//   // apiGetElementsByIds, // ⛔️ no longer needed if details are local; keep if you still use it elsewhere
// } from "@/lib/composeApi";
// import {
//   getFoundIds,
//   PROGRESS_EVT,
...
// // If you already moved element details to local cache, import and use these:
// // import {
...
//   // load progress + refresh when crafting progresses
//   useEffect(() => {
//     const loadFound = () => setFoundSet(new Set(getFoundIds()));
//     loadFound();
//     window.addEventListener(PROGRESS_EVT, loadFound);
//     return () => window.removeEventListener(PROGRESS_EVT, loadFound);
//   }, []);

//   // fetch base elements (with game_code)
//   useEffect(() => {
//     let cancelled = false;
//     (async () => {
...
//         const items = (res.items ?? []).map((it: any) => ({ ...it, id: Number(it.id) })) as ElementDto[];
//         if (!cancelled) {
//           setBaseElements(items);
//         }
//       } catch (err) {
//         console.error("[BASES] failed:", err);
//         if (!cancelled) setBaseElements([]);
//       } finally {
...
//   // fetch full details for found IDs (with game_code)
//   // If you've moved to local cache, replace this block with local lookups.
//   useEffect(() => {
//     if (!foundSet) return;
//     const ids = [...foundSet];
//     if (ids.length === 0) {
...
//         // const res = await apiGetElementsByIds(ids);
//         // const items = (res.items ?? []).map((it: any) => ({ ...it, id: Number(it.id) })) as ElementDto[];

//         // If you switched to local cache, do something like:
//         // const items: ElementDto[] = ids.map(id => getElementLocal(id)).filter(Boolean) as ElementDto[];

//         // For now, keep old API call commented out (uncomment if you have it implemented):
//         // -------------------------------------------
//         // const res = await apiGetElementsByIds(ids);
//         // const items = (res.items ?? []).map((it: any) => ({ ...it, id: Number(it.id) })) as ElementDto[];
//         // -------------------------------------------

//         // If you don't have /by-ids implemented yet, you can just keep foundDetails unchanged here,
//         // because the instant-prune effect below will still hide finished challenges immediately.
//         const items: ElementDto[] = []; // ← replace with your actual source (API or local)

//         if (!cancelled) {
//           setFoundDetails(items);
//         }
//       } catch (err) {
//         console.error("[FOUND DETAILS] failed:", err);
//         if (!cancelled) setFoundDetails([]);
//       } finally {
//         if (!cancelled) setFoundDetailsLoading(false);
//       }
//     })();
//     return () => { cancelled = true; };
//   }, [foundSet]);

//   // challenges: POST discovered_character_ids (with game_code)
...
//   useEffect(() => {
//     if (!foundSet) return;
//     if (foundDetailsLoading) return; // wait until found details resolved at least once
//     refreshChallenges(foundSet);
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [foundSet, foundDetailsLoading]);

//   // ✅ NEW: instant-prune any challenge that is already found (no reload, no refetch needed)
//   useEffect(() => {
//     if (!foundSet) return;
//     setChallenges((prev) => prev.filter((c) => !foundSet.has(c.id)));
//   }, [foundSet]);

//   // If a challenge just got found, prune & refill (kept from your version)
//   useEffect(() => {
...
//       // optional re-fetch to pull next batch from server
//       refreshChallenges(foundSet);
//     }
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [foundSet, challenges.length]);

//   // Collections (found characters only)
//   const collections = useMemo(() => {
//     if (!foundSet) return [];
//     return foundDetails.filter((d) => d.is_character && foundSet.has(d.id));
//   }, [foundSet, foundDetails]);

//   // Inventory = bases (backend) + discovered non-character elements (backend-foundDetails)
...
//                   No challenges available (check seeding for this game_code).
//                 </div>
//               )}
...
export default function SidePanel({ onSpawn }: { onSpawn: (id: number) => void }) {
  ...
  // refetch challenges when relevant local data stabilizes
  useEffect(() => {
    if (!foundSet) return;
  ...
  // server-side prune + refill (kept from your logic)
  useEffect(() => {
    if (!foundSet || challenges.length === 0) return;
    const newlyFound = challenges.filter((c) => foundSet.has(c.id));
    if (newlyFound.length > 0) {
                ...
                <div className="grid gap-2">
                  {collections.map((b) => (
                    <div key={b.id} className="flex items-center gap-2 bg-white/5 p-2 rounded">
                      ...
                      <div className="text-sm">
                        <div className="font-semibold">{b.name}</div>
                        <div className="text-xs opacity-70">~{b.difficulty}% players discovered</div>
                      </div>
                    </div>
                  ))}
                </div>
