...
// src/components/CraftingScene.tsx
// "use client";
...
//   React.useEffect(() => {
//     (async () => {
//       await initBaseIfNeededLocal(); // one-time base pull to local
//     })();
//   }, []);

//   React.useEffect(() => {
//     if (!spawnRequest) return;
//     // only spawn if we have the element in local cache (no network)
//     const exists = getElementLocal(spawnRequest.id);
//     if (!exists) return; // safety guard
//     setCards(prev => [
//       ...prev,
//       {
...
//       // ✅ SUCCESS: write result to LOCAL cache AND record progress
//       const result = res.result as ElementDto;
//       upsertElementLocal(result, true);   // updates cache + fires COMPOSE_CACHE_UPDATED
//       addFoundId(result.id);              // fires your PROGRESS_EVT
...
//                 {/* Prefer full element if available; fallback to minimal */}
