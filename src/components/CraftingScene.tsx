...
//   // NEW: which two cards are currently in "hold to combine" state (for scale effect)
//   const [holdPair, setHoldPair] = React.useState<{ sourceId: string; targetId: string } | null>(
//     null
//   );
...
//   // Spawn from inventory clicks, with multi-row layout
//   React.useEffect(() => {
//     if (!spawnRequest) return;
//     const exists = getElementLocal(spawnRequest.id);
//     if (!exists) return;
...
//         }
//         setCards((prev) =>
//           prev.map((c) => (c.id === sourceId ? { ...c, shake: true } : c))
//         );
//         setTimeout(
//           () =>
//             setCards((prev) =>
//               prev.map((c) => (c.id === sourceId ? { ...c, shake: false } : c))
//             ),
//           500
//         );
...
//         const tr = rect(te);
//         const mid =
//           sr && tr ? { x: (sr.left + tr.left) / 2, y: (sr.top + tr.top) / 2 } : undefined;
//         if (mid) addCard(result.id, mid);
//         else addCard(result.id);
//       }
//     } catch (e) {
//       console.error("[combine] failed:", e);
//     } finally {
...
//       }
//       setHoldUI(null);
//       setHoldPair(null); // clear highlight pair
//       isCombiningRef.current = false;
//     }
//   }
...
//       setCards((prev) =>
//         prev.map((c) => (c.id === spawnId ? { ...c, x: next.x, y: next.y } : c))
//       );

//       const stage = stageRef.current!;
//       const dragged = stage.querySelector(
//         `[data-spawn-id="${spawnId}"]`
//       ) as HTMLElement | null;
//       const db = rect(dragged);
//       if (!db) return;
...
//           // NEW: highlight both cards while holding
//           setHoldPair({ sourceId: spawnId, targetId: best.id });

//           const ringTick = () => {
//             const elapsed = Date.now() - startTs;
//             const pct = Math.min(100, (elapsed / HOLD_MS) * 100);
//             setHoldUI({
//               x: best!.r.left + best!.r.width / 2 - 18,
//               y: best!.r.top - 42,
//               pct,
//             });
//             if (pct < 100 && holdRef.current?.targetId === best!.id) {
...
//         }
//         setHoldUI(null);
//         setHoldPair(null); // no overlapping target → no highlight
//       }
//     };
...
//       window.removeEventListener("mouseup", endDrag as any);
//       window.removeEventListener("blur", endDrag as any);
//       (draggable as HTMLElement).removeEventListener(
//         "lostpointercapture",
//         endDrag as any
//       );
...
//         >
//           {cards.map((c) => {
//             const isHoldHighlighted =
//               holdPair &&
//               (c.id === holdPair.sourceId || c.id === holdPair.targetId);
...
//                     isHoldHighlighted ? "scale-110 shadow-[0_0_16px_rgba(255,255,255,0.5)]" : "",
//                   ]
//                     .filter(Boolean)
//                     .join(" ")}
//                 >
//                   <ElementCard
//                     el={(getElementLocal(c.elementId) ?? { id: c.elementId }) as any}
//                   />
//                 </div>
//               </div>
//             );
...
//           {cards.length === 0 && (
//             <div className="absolute inset-0 flex items-center justify-center opacity-60 text-white/80 text-sm">
//               Open <b className="mx-1">Inventory</b>, spawn two elements, then drag and hold one
//               over the other to craft.
//             </div>
//           )}
...
export default function CraftingScene({ spawnRequest }: { spawnRequest?: SpawnRequest }) {
  ...
  // which two cards are currently in "hold to combine" state (for scale effect)
  const [holdPair, setHoldPair] = React.useState<{ sourceId: string; targetId: string } | null>(
    null
  );
  ...
  async function attemptCombineLocked(sourceId: string, targetId: string) {
    ...
    try {
      ...
      if (!res || !res.result) {
        // FAIL feedback + sound (✖ near dragged card)
        const se = getSpawnEl(sourceId);
        const sr = rect(se);
        if (sr) {
          const sc = center(sr);
          setFailBadge({ x: sc.x - 12, y: sc.y - 12 }); // closer to cursor / dragged card
          setTimeout(() => setFailBadge(null), 700);
        }
        setCards((prev) =>
          prev.map((c) => (c.id === sourceId ? { ...c, shake: true } : c))
    ...
    const onMove = (ev: PointerEvent) => {
      ...
      if (best) {
        if (holdRef.current?.targetId !== best.id) {
          ...
          // highlight both cards while holding
          setHoldPair({ sourceId: spawnId, targetId: best.id });

          // ring anchored near dragged card center (closer to cursor)
          const ringTick = () => {
            const elapsed = Date.now() - startTs;
            const pct = Math.min(100, (elapsed / HOLD_MS) * 100);
            setHoldUI({
              x: dbCenter.x - 18,
              y: dbCenter.y - 40, // closer to cursor than before
              pct,
            });
            if (pct < 100 && holdRef.current?.targetId === best!.id) {
              requestAnimationFrame(ringTick);
  ...
  return (
    <>
      <section className="pt-16 pr-[320px] pl-4 min-h-screen">
        <div
          ref={stageRef}
          className="relative mx-auto max-w-4xl min-h-[72vh] rounded-xl bg-black/80 border-2 border-white/30 overflow-hidden"
        >
          {cards.map((c) => {
            const isHoldHighlighted =
              holdPair &&
